...
func TestWriter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			stdout := bytes.Buffer{}
			stderr := bytes.Buffer{}
			encTar, err := io.ReadAll(&testData.encOutput)
			assert.NoError(t, err)
			...
			for {
				...
				bs, err := io.ReadAll(resultReader)
...
func ExampleWriter_AddFile() {
	...
	if err := os.WriteFile(fileToAdd.Name(), []byte("Hello world"), os.FileMode(0755)); err != nil {
		panic(err)
	}
